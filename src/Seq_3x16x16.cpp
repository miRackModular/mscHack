...
using api0::PulseGenerator;
using api0::SchmittTrigger;
using api0::randomUniform;
using api0::SVGFader;
using api0::engineGetSampleRate;
...
Seq_3x16x16_Widget::Seq_3x16x16_Widget( Seq_3x16x16 *module ) : ModuleWidget(module) 
{
    ...
    setPanel(APP->window->loadSvg(asset::plugin( thePlugin, "res/Seq_3x16x16.svg")));
...
void Seq_3x16x16::step() 
{
	...
	for( ch = 0; ch < CHANNELS; ch++ ) 
    {
        ...
        // trigger
        outputs[ OUT_GATES + ch ].value = bChTrig ? CV_MAX10 : 0.0;
    }
...
Model *modelSeq_3x16x16 = Model::create<Seq_3x16x16, Seq_3x16x16_Widget>( "mscHack", "Seq_3ch_16step", "SEQ 3 x 16", api0::SEQUENCER_TAG, api0::MULTIPLE_TAG );
